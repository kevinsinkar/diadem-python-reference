...
description: "Specifies the user command which DIAdem REPORT triggers when you click a 2D axis system while pressing a key. The user command receives a parameter. The paramet"
---

# IRepReportOnClickedWithKeyInt.OnAxisSystem2D

!!! abstract "Property &middot; `ReportApi.chm`"
    Property: OnAxisSystem2D for ClickedWithKeyEvents

Specifies the user command which DIAdem REPORT triggers when you click a 2D axis system while pressing a key. The user command receives a parameter. The parameter corresponds with a D2AxisSystemClickedWithKeyContext object and provides information about the 2D axis system which you clicked while pressing a key. For this event DIAdem does not support keys such as <Alt>, <Ctrl>, and <Shift>. Use the methods ConvertPageXPositionToXAxisValue and ConvertPageYPositionToYAxisValue to convert the page coordinates into coordinates of a 2D axis system.
...
*Source: `ReportApi/properties/Report_property_OnAxisSystem2D_IRepReportOnClickedWithKeyInt.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
