---
title: "ToVirtGridCtrl.LockUpdate"
description: "Specifies in user dialog boxes that the control display does not refresh. For example, if you add rows to a table in a script, you can use this property to prev"
---

# ToVirtGridCtrl.LockUpdate

!!! abstract "Property &middot; `Sudref.chm`"
    Property: LockUpdate for XTable

Specifies in user dialog boxes that the control display does not refresh. For example, if you add rows to a table in a script, you can use this property to prevent the table from flickering.
...
<div markdown="1">
<table class="Borderless" id="table2"><tr><td class="Icon"><img src="../image/note.gif"/></td><td><strong>Note  </strong>If you set this property within a script, reset the property before the end of the script.</td></tr></table>
</div>
...
def Button1_EventClick(ByRef This):
    ...
    XTable1.LockUpDate = TRUE
    XTable1.Columns.Add("NewCol")
    Width = XTable1.Width - XTable1.RowTitlesWidth
    N = XTable1.Columns.Count
    for T in range(1, N-1 + 1):
        Width = Width-XTable1.Columns(T).Width  Next
        XTable1.Columns(N).Width = Width
        XTable1.LockUpdate = FALSE
        XTable1.Refresh()
```
...
*Source: `Sudref/properties/SUD_property_LockUpdate_ToVirtGridCtrl.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
