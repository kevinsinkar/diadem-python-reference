---
title: "ToCurvePreviewOb.ObjectCode"
description: "Specifies the unique name for a control in a user dialog box. Use this name to select a control in the dialog editor, to edit its properties and events. In scri"
---

# ToCurvePreviewOb.ObjectCode

!!! abstract "Property &middot; `Sudref.chm`"
    Property: ObjectCode for Curve2DPreview

Specifies the unique name for a control in a user dialog box. Use this name to select a control in the dialog editor, to edit its properties and events. In scripts you also use the ObjectCode of a control to access the control. Do not use the names of DIAdem variables or DIAdem functions as ObjectCode , otherwise you will no longer be able to use them in DIAdem.
...
```python
Curve2DPreview1.Height = 220
```

```python
for i in range(1, Dialog.Controls.Count + 1):
    MsgBoxDisp("ObjectCode: " + Dialog.Controls(i).ObjectCode)
```
...
*Source: `Sudref/properties/SUD_property_ObjectCode_ToCurvePreviewOb.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
