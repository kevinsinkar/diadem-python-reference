---
title: "ToTableCtrl.SelectedCells"
description: "Returns a collection of the selected cells in user dialog boxes. Access single cells in scripts using the Item property or by entering the index in parentheses."
---

# ToTableCtrl.SelectedCells

!!! abstract "Property &middot; `Sudref.chm`"
    Property: SelectedCells for Table

Returns a collection of the selected cells in user dialog boxes. Access single cells in scripts using the Item property or by entering the index in parentheses.

## Signature

```python
return_value = obj.SelectedCells
```
...
```python
MsgBoxDisp(Table1.SelectedCells.Count)
```

```python
MsgBoxDisp(Table1.SelectedCells.Item(1).Value)
```

```python
MsgBoxDisp(Table1.SelectedCells(1).Value)
```
...
*Source: `Sudref/properties/SUD_property_SelectedCells_ToTableCtrl.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
