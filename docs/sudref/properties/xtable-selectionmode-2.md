---
title: "XTable.SelectionMode"
description: "Specifies whether the selection list in an extended table allows multiple selection. The property can have the following settings: 0 - Single selection Multiple"
---

# XTable.SelectionMode

!!! abstract "Property &middot; `Sudref.chm`"
    Property: SelectionMode for ListBox <XTable>

Specifies whether the selection list in an extended table allows multiple selection. The property can have the following settings: 0 - Single selection Multiple selection is not possible. 1 - Multiselection You can make a multiple selection in any order.
...
<td><strong>Note  </strong>A variable binding does not support multiple selection. If you connect a selection list that has multiple selection to a variable, only the first entry in the selection list that corresponds to the value of the variable, appears when the dialog box opens. If you select several entries in a selection list, the connected variable receives only the value of the last selected entry.</td>
</tr>
</table>
</div>
...
*Source: `Sudref/properties/SUD_property_selectionmode_ToListBoxRt_XTable.htm`*
