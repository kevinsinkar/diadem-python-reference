---
title: "ToListItem.InterpretedText"
description: "Returns the interpreted selection term in a selection list in user dialog boxes. The user dialog box interprets texts with DIAdem expressions . DIAdem expressio"
---

# ToListItem.InterpretedText

!!! abstract "Property &middot; `Sudref.chm`"
    Property: InterpretedText for ListItem

Returns the interpreted selection term in a selection list in user dialog boxes. The user dialog box interprets texts with DIAdem expressions . DIAdem expressions are DIAdem variables or formula expressions enclosed in @ , or script expressions from registered DIAdem scripts, enclosed in @@ .
...
<td><strong>Note</strong>  The property returns an interpreted text only if you assign the value  <span class="Monospace">1–Yes</span> to the <span class="Monospace">InterpretedTextEnabled</span> property.</td>
</tr>
</table>
</div>
...
```python
MsgBoxDisp(ListBox1.Items(1).InterpretedText)
```
...
*Source: `Sudref/properties/SUD_property_InterpretedText_ToListItem.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
