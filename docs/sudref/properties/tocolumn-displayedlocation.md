---
title: "ToColumn.DisplayedLocation"
description: "Returns the index where a table displays a column in a user dialog box."
---

# ToColumn.DisplayedLocation

!!! abstract "Property &middot; `Sudref.chm`"
    Property: DisplayedLocation for Column

Returns the index where a table displays a column in a user dialog box.

## Signature

```python
obj.DisplayedLocation
```
...
<td><strong>Note  </strong>If you enter the position using the <a href="../totablectrl-columns/">columns</a> or <a href="../totablectrl-rows/">rows property</a>, you refer to the original table order. The order remains unchanged when you move columns or rows interactively, or when you use the <a href="../../methods/tocolumnsdisp-swap/">Swap</a> method. The <span class="Monospace">DisplayedLocation</span> property contains the position where the table displays the row or column.</td>
</tr>
</table>
</div>
...
```python
MsgBoxDisp(Table1.Columns(1).DisplayedLocation)
# The value 1 is displayed
Table1.Columns.Swap(1, 3)
MsgBoxDisp(Table1.Columns(1).DisplayedLocation)
# After the exchange the value 3 is displayed
```
...
*Source: `Sudref/properties/SUD_property_DisplayedLocation_ToColumn.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
