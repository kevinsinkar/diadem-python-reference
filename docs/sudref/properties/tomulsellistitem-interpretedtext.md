---
title: "ToMulSelListItem.InterpretedText"
description: "Returns the interpreted text of a term that is selected in the selection list, in user dialog boxes. The user dialog box interprets texts with DIAdem expression"
---

# ToMulSelListItem.InterpretedText

!!! abstract "Property &middot; `Sudref.chm`"
    Property: InterpretedText for ListItem

Returns the interpreted text of a term that is selected in the selection list, in user dialog boxes. The user dialog box interprets texts with DIAdem expressions . DIAdem expressions are DIAdem variables or formula expressions enclosed in @ , or script expressions from registered DIAdem scripts, enclosed in @@ .
...
<td><strong>Note</strong>  The property returns an interpreted text only if you assign the value  <span class="Monospace">1–Yes</span> to the <a href="../tolistboxrt-interpretedtextenabled/">InterpretedTextEnabled</a> property.</td>
</tr>
</table>
</div>
...
```python
for i in range(1, ListBox1.MultiSelection.Count + 1):
    MsgBoxDisp(ListBox1.MultiSelection.Item(i).Index + " , " + ListBox1.MultiSelection.Item(i).InterpretedText)
```
...
*Source: `Sudref/properties/SUD_property_InterpretedText_ToMulSelListItem.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
