---
title: "ToTableCtrl.Columns"
description: "Returns a collection of all the columns in a table in user dialog boxes. Access single columns in scripts using the Item property or by entering the column inde"
---
...
```python
Table1.Columns(1).Title = "Column 1"
```

```python
Table1.Columns.Item(1).Cells(2).Value = 25
```

```python
MsgBoxDisp(Table1.Columns.Count)
```
