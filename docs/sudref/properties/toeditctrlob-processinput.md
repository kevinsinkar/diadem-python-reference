---
title: "ToEditCtrlOb.processinput"
description: "Specifies in user dialog boxes whether an expression is interpreted when a DIAdem textbox is exited. The property can have the following settings: 0 - No DIAdem"
---

# ToEditCtrlOb.processinput

!!! abstract "Property &middot; `Sudref.chm`"
    Property: ProcessInput for EditBox

Specifies in user dialog boxes whether an expression is interpreted when a DIAdem textbox is exited. The property can have the following settings: 0 - No DIAdem does not interpret the expression. 1 - Yes DIAdem interprets the expression if this expression can be directly converted into a number or a text, for example, Pi , 17+4 , sin(55.4) , or T1 . DIAdem cannot evaluate more complex VBS syntax or user commands.
...
*Source: `Sudref/properties/SUD_property_processinput_ToEditCtrlOb.htm`*
