---
title: "ToTreeCtrl.DragAllowed"
description: "Makes it possible to drag selected elements from the tree. The associated EventDragStart event determines which data is transported. The default setting is fals"
---

# ToTreeCtrl.DragAllowed

!!! abstract "Property &middot; `Sudref.chm`"
    Property: DragAllowed for Tree

Makes it possible to drag selected elements from the tree. The associated EventDragStart event determines which data is transported. The default setting is false so that the event is not called unnecessarily.
...
```python
oMyReportObj = Dialog <NonModal>.GetControl
MsgBoxDisp(oMyReportObj.DragAllowed)
```
...
*Source: `Sudref/properties/SUD_property_DragAllowed_ToTreeCtrl.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
