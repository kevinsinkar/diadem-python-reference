---
title: "ToTreeCtrl.Nodes"
description: "Specifies in user dialog boxes the tree nodes that are on the first level and are also called root nodes. Each root node has its own node collection which produ"
---

# ToTreeCtrl.Nodes

!!! abstract "Property &middot; `Sudref.chm`"
    Property: Nodes for Tree
...
Specifies in user dialog boxes the tree nodes that are on the first level and are also called root nodes. Each root node has its own node collection which produces the display hierarchy of the tree. Use the Nodes for TreeNode property to access a node of the next subordinate level within a tree branch.

## Signature

```python
return_value = obj.Nodes
```
...
*Source: `Sudref/properties/SUD_property_Nodes_ToTreeCtrl.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
