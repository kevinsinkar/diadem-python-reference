---
title: "ToTableCtrl.Rows"
description: "Returns a collection of all the rows of a table in user dialog boxes. Access single rows in scripts using the Item property or by entering the row index in pare"
---
...
Returns a collection of all the rows of a table in user dialog boxes. Access single rows in scripts using the Item property or by entering the row index in parentheses. Click Rows in the Properties tab to edit the properties of the table rows in the dialog editor.

## Signature

```python
return_value = obj.Rows
```
...
```python
Table1.Rows(1).Title = "Row 1"
```

```python
Table1.Rows.Item(1).Cells(2).Value = 25
```

```python
MsgBoxDisp(Table1.Rows.Count)
```
