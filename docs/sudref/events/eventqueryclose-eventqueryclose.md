---
title: "eventqueryclose.EventQueryClose"
description: "Triggers in user dialog boxes if the methods Dialog. OK and Dialog. Cancel are called, or if the OK button, the Cancel button, or the Esc button are pressed. Th"
---

# eventqueryclose.EventQueryClose

!!! abstract "Event &middot; `Sudref.chm`"
    Event: EventQueryClose for Dialog

Triggers in user dialog boxes if the methods Dialog. OK and Dialog. Cancel are called, or if the OK button, the Cancel button, or the Esc button are pressed. This event also triggers when an error occurs in a user dialog box or when DIAdem closes.

## Signature

```python
dd.EventQueryClose(ByRef This, ByRef bCanClose, Reason)
```
...
*Source: `Sudref/events/sud_event_eventqueryclose.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
