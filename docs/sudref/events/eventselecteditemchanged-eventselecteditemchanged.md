---
title: "EventSelectedItemChanged.EventSelectedItemChanged"
description: "Is triggered in user dialog boxes if the selected node in the tree changes. Use the SelectedItem property to request the current node. You can prevent the selec"
---

# EventSelectedItemChanged.EventSelectedItemChanged

!!! abstract "Event &middot; `Sudref.chm`"
    Event: EventSelectedItemChanged for Tree

Is triggered in user dialog boxes if the selected node in the tree changes. Use the SelectedItem property to request the current node. You can prevent the selection by assigning Nothing to the node returned by SelectedItem . You cannot select several nodes.

## Signature

```python
dd.EventSelectedItemChanged(ByRef This)
```
...
*Source: `Sudref/events/sud_event_EventSelectedItemChanged.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
