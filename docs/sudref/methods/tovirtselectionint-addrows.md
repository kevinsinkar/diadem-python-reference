---
title: "ToVirtSelectionInt.AddRows"
description: "Selects the specified rows of an extended table in user dialog boxes. If multiple selection is possible in the extended table, this method adds the specified ro"
---

# ToVirtSelectionInt.AddRows

!!! abstract "Method &middot; `Sudref.chm`"
    Method: AddRows for Selection

Selects the specified rows of an extended table in user dialog boxes. If multiple selection is possible in the extended table, this method adds the specified rows to the rows that are already selected.

## Signature

```python
obj.AddRows(FromRow, ToRow)
```
...
XTable1.SelectedElements.AddRows(2,3)
XTable1.Refresh
```
