---
title: "ToSudViewObInt.RunCustomActionAll"
description: "Triggers the EventCustomAction event for a dialog box and all contained controls. The method triggers the event from the calling level, for each control of the "
---

# ToSudViewObInt.RunCustomActionAll

!!! abstract "Method &middot; `Sudref.chm`"
    Method: RunCustomActionAll for Dialog <NonModal>
...
Triggers the EventCustomAction event for a dialog box and all contained controls. The method triggers the event from the calling level, for each control of the lower levels. You can use this event anywhere, for any purpose.

## Signature

```python
obj.RunCustomActionAll(VarParam)
```
...
sMyAction = "StoreData"
oMyDlg.RunCustomActionAll(sMyAction)
```
...
*Source: `Sudref/methods/SUD_method_RunCustomActionAll_ToSudViewObInt.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
