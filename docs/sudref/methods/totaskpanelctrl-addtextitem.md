---
title: "ToTaskPanelCtrl.AddTextItem"
description: "Creates a new entry in a group in the TaskPanel. This entry is not selectable and cannot be linked to an action but can only display a text."
---

# ToTaskPanelCtrl.AddTextItem

!!! abstract "Method &middot; `Sudref.chm`"
    Method: AddTextItem for TaskPanel

Creates a new entry in a group in the TaskPanel. This entry is not selectable and cannot be linked to an action but can only display a text.

## Signature

```python
obj.AddTextItem(GroupKey, Key, Text)
```
...
*Source: `Sudref/methods/SUD_method_AddTextItem_ToTaskPanelCtrl.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
