---
title: "ToChnListItems.Item"
description: "Returns the ListItem object for a specific index in user dialog boxes. The returned object enables you to access the entries in the collection."
---

# ToChnListItems.Item

!!! abstract "Method &middot; `Sudref.chm`"
    Method: Item for ChnListItems
...
Returns the ListItem object for a specific index in user dialog boxes. The returned object enables you to access the entries in the collection.
...
<tr>
<td class="Icon"><img src="../image/note.gif"/></td>
<td><strong>Note</strong>  If you assign the value <span class="Monospace">1</span> to the <span class="Monospace">ShowBlankEntry</span> property, the channel selection list shows a blank entry as the first entry. The index then starts with the value <span class="Monospace">0</span>. If the channel selection list does not show a blank entry, the index starts at <span class="Monospace">1</span>.</td>
</tr>
</table>
</div>
...
```python
MsgBoxDisp(ChnComboBox1.Items.Item(1).Value)
MsgBoxDisp(CHnComboBox1.Items(1).Value)
```
...
*Source: `Sudref/methods/SUD_method_Item_ToChnListItems.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
