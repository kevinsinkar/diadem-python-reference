---
title: "ToSelectionRangeInt.istable"
description: "Specifies in user dialog boxes whether the current selection block in the extended table contains the whole table. You can only select the whole extended table "
---

# ToSelectionRangeInt.istable

!!! abstract "Method &middot; `Sudref.chm`"
    Method: IsTable for SelectionItem

Specifies in user dialog boxes whether the current selection block in the extended table contains the whole table. You can only select the whole extended table with a click in the top left cell, when the property AllowTable has the value True .

## Signature

```python
bIsTable = Object.IsTable()
```
...
*Source: `Sudref/methods/SUD_method_istable_ToSelectionRangeInt.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
