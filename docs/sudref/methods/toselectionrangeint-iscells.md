---
title: "ToSelectionRangeInt.iscells"
description: "Specifies in user dialog boxes whether only adjacent cells are selected in the current selection block of the extended table, but not complete rows or columns."
---

# ToSelectionRangeInt.iscells

!!! abstract "Method &middot; `Sudref.chm`"
    Method: IsCells for SelectionItem

Specifies in user dialog boxes whether only adjacent cells are selected in the current selection block of the extended table, but not complete rows or columns.

## Signature

```python
bIsCells = Object.IsCells()
```
