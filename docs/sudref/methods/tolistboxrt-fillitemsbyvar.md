---
title: "ToListBoxRt.FillItemsByVar"
description: "Adds the selection terms from an enumeration variable to an existing selection list. The selection terms can be appended to the list as script terms or as user "
---

# ToListBoxRt.FillItemsByVar

!!! abstract "Method &middot; `Sudref.chm`"
    Method: FillItemsByVar for ListBox

Adds the selection terms from an enumeration variable to an existing selection list. The selection terms can be appended to the list as script terms or as user interface terms.

## Signature

```python
obj.FillItemsByVar(Variable, Translate)
```
...
```python
ListBox1.Items.RemoveAll
ListBox1.FillItemsByVar("XChnStyle",FALSE)
```
...
*Source: `Sudref/methods/SUD_method_FillItemsByVar_ToListBoxRt.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
