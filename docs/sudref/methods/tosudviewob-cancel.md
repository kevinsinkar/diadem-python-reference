---
title: "ToSudViewOb.Cancel"
description: "Triggers the EventTerminate event and closes the user dialog box. The method has the same effect as clicking a button that has the value Cancel for the ButtonTy"
---

# ToSudViewOb.Cancel

!!! abstract "Method &middot; `Sudref.chm`"
    Method: Cancel for Dialog

Triggers the EventTerminate event and closes the user dialog box. The method has the same effect as clicking a button that has the value Cancel for the ButtonType property. The method sets the value of the DlgState DIAdem variable as IDCancel .
...
<td><strong>Note  </strong>Select <strong>Edit»Variables</strong> and the <strong>Restore when cancels</strong> property for the DIAdem variables defined in the user dialog box, to restore the original values after you cancel.</td>
</tr>
<tr><td class="Icon"><img src="../image/note.gif"/></td><td><strong>Note  </strong>If this method is called in a Sub procedure, the procedure does not automatically terminate. If the <span class="Monospace">Cancel</span> method call is not the last statement in the Sub procedure, use the<span class="Monospace"> Exit Sub</span> statement after the <span class="Monospace">Cancel</span> method call, to terminate the Sub procedure.</td></tr>
<tr><td class="Icon"><img src="../image/note.gif"/></td><td><strong>Note</strong>  The user dialog box does not close until the subprocedure that contains the call for the  <span class="Monospace">Cancel</span> method has been completely executed. Use the <a href="../tosudviewob-hide/">Hide</a> method if you want to hide the dialog box at an earlier point in time.</td></tr></table>
</div>
