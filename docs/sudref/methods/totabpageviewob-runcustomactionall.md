---
title: "ToTabPageViewOb.RunCustomActionAll"
description: "Triggers the EventCustomAction event for a tab and all contained controls. The method triggers the event from the calling level, for each control of the lower l"
---

# ToTabPageViewOb.RunCustomActionAll

!!! abstract "Method &middot; `Sudref.chm`"
    Method: RunCustomActionAll for Page

Triggers the EventCustomAction event for a tab and all contained controls. The method triggers the event from the calling level, for each control of the lower levels. You can use this event anywhere, for any purpose.
...
def Button1_EventClick(ByRef This):
    sMyAction = "StoreData"
    Page1.RunCustomActionAll(sMyAction)
```
...
*Source: `Sudref/methods/SUD_method_RunCustomActionAll_ToTabPageViewOb.HTM`&nbsp;&middot;&nbsp;Python translated from VBS*
