---
title: "ToTaskPanelCtrl.SetPictureIndex"
description: "Specifies in user dialog boxes the index of the sub-graphic that DIAdem displays on an entry in the TaskPanel. The user dialog box divides the graphic you speci"
---

# ToTaskPanelCtrl.SetPictureIndex

!!! abstract "Method &middot; `Sudref.chm`"
    Method: SetPictureIndex for TaskPanel

Specifies in user dialog boxes the index of the sub-graphic that DIAdem displays on an entry in the TaskPanel. The user dialog box divides the graphic you specify with the Picture property into several smaller sub-graphics with the specified width PictureWidth , and assigns these sub-graphics to the respective index. The first sub-graphic has the index 1. If you do not specify an index, the index has the value 0. You only need to specifiy an index if you want to use a graphic or if you want to delete a currently displayed graphic. A group can also display a graphic. Not all entries need to be assigned to a graphic.

## Signature

```python
bSetPictureIndex = Object.SetPictureIndex(Key, Index)
```
...
def CreateDefaultTaskPanel(ByRef This):
    ...
    This.SetPictureIndex("handtoolGroup",1)
    This.AddGroup("textonlyGroup", "Info")
    This.SetPictureIndex("textonlyGroup",3)
    ...
    This.AddLinkItem("handtoolGroup","screwdriver","Screw driver")
...
*Source: `Sudref/methods/SUD_method_SetPictureIndex_ToTaskPanelCtrl.htm`&nbsp;&middot;&nbsp;Python translated from VBS*
