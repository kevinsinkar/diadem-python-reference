---
title: "BlEvntSourceGrp"
description: "Specifies the number of the signal output that the alarm signal, which is connected to the alarm input, originates from."
---

# BlEvntSourceGrp

!!! abstract "Variable &middot; `VarOnl.chm`"
    Variable: BlEvntSourceGrp

Specifies the number of the signal output that the alarm signal, which is connected to the alarm input, originates from.
...
<donottranslate><pre>Call DACObjOpen(<em>BlockName</em>)
    BlEvntSourceGrp(i,j) = <em>Value</em>
...
*Source: `VarOnl/BlEvntSourceGrp.htm`*
