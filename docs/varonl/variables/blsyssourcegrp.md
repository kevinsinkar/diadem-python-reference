---
title: "BlSysSourceGrp"
description: "Specifies the number of the signal output that the clock signal, which is connected to the system input, originates from."
---

# BlSysSourceGrp

!!! abstract "Variable &middot; `VarOnl.chm`"
    Variable: BlSysSourceGrp

Specifies the number of the signal output that the clock signal, which is connected to the system input, originates from.
...
<donottranslate><pre>Call DACObjOpen(<em>BlockName</em>)
    BlSysSourceGrp(i,j) = <em>Value</em>
...
*Source: `VarOnl/BlSysSourceGrp.htm`*
