---
title: "DCSUDSTesterPresentServiceExecute"
description: "Executes a Tester Present Service to keep the engine control unit (ECU) in the diagnostic mode."
---

# DCSUDSTesterPresentServiceExecute

!!! abstract "Command &middot; `ComOnl.chm`"
    Command: DCSUDSTesterPresentServiceExecute

Executes a Tester Present Service to keep the engine control unit (ECU) in the diagnostic mode.

## Signature

```python
dd.DCSUDSTesterPresentServiceExecute(DCSDiagRef, DCSResponseRequired)
```

## Parameters

<div markdown="1">
<table class="Borderless">
<tr><td width="150">DCSDiagRef</td>
<td>Specifies the diagnostic handle in a vehicle.<div id="exp_DCSDiagRef">
<table class="Borderless">
<tr><td><a href="#" data-unresolved="1">Longinteger variable</a></td></tr>
<tr>
<td>Access: Read only</td></tr>
</table>
</div></td></tr>
<tr><td width="150">DCSResponseRequired</td>
<td>Specifies whether a service requires a response.<div id="exp_DCSResponseRequired">
<table class="Borderless">
<tr><td><a href="#" data-unresolved="1">Boolean Variable</a></td></tr>
<tr>
<td>Access: Read only</td></tr>
</table>
</div></td></tr>
</table>
</div>

---

*Source: `ComOnl/DCSUDSTesterPresentServiceExecute.htm`*
