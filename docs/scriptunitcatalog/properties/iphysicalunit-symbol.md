---
title: "IPhysicalUnit.Symbol"
description: "Returns the enabled unit symbol of the units catalog. The active symbol of a unit is the symbol that DIAdem displays by default for a specific unit, in all the "
---

# IPhysicalUnit.Symbol

!!! abstract "Property &middot; `ScriptUnitCatalog.chm`"
    Property: Symbol for Unit

Returns the enabled unit symbol of the units catalog. The active symbol of a unit is the symbol that DIAdem displays by default for a specific unit, in all the dialog boxes of the units catalog.

## Signature

```python
return_value = obj.Symbol
```

## Python example

```python
dd.MsgBoxDisp (dd.UnitCatalog.Quantities("volume").Units("liter").Symbol)
```
...
*Source: `ScriptUnitCatalog/properties/DiaCmpnt_property_Symbol_IPhysicalUnit.htm`*
