---
title: "IUnitSetQuantityCollection.Count"
description: "Returns the number of units that are used by one physical quantity, for a unit set in the units catalog."
---

# IUnitSetQuantityCollection.Count

!!! abstract "Property &middot; `ScriptUnitCatalog.chm`"
    Property: Count for UsedQuantities

Returns the number of units that are used by one physical quantity, for a unit set in the units catalog.
...
```python
dd.MsgBoxDisp (dd.UnitCatalog.Sets("mechanics").UsedQuantities.Count)
```
...
*Source: `ScriptUnitCatalog/properties/DiaCmpnt_property_Count_IUnitSetQuantityCollection.htm`*
