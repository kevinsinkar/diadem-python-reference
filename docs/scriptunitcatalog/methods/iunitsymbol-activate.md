---
title: "IUnitSymbol.Activate"
description: "Makes the selected symbol the enabled symbol of a unit. DIAdem displays only the enabled symbols, in all the units catalog dialog boxes."
---

# IUnitSymbol.Activate

!!! abstract "Method &middot; `ScriptUnitCatalog.chm`"
    Method: Activate for Symbol

Makes the selected symbol the enabled symbol of a unit. DIAdem displays only the enabled symbols, in all the units catalog dialog boxes.
...
```python
dd.UnitCatalog.Sets("mechanics").UsedQuantities("volume").UsedUnits("liter").SymbolAliases(1).Activate()
```
...
*Source: `ScriptUnitCatalog/methods/DiaCmpnt_method_Activate_IUnitSymbol.htm`*
