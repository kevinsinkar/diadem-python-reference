...
!!! abstract "Command &middot; `ComOff.chm`"
    Command: ChnNonLinearFitY

Uses the Levenberg-Marquardt algorithm to specify the least squares of the coefficients a , which best fit the input pairs, which means they also best fit a non-linear function of the y = f(x,a) type.

## Signature

```python
dd.ChnNonLinearFitY( XW , Y , SamplingPointChn, ResultChannel , NonLinearFitMaxIterations, NonLinearFitCovariance)
```
...
*Source: `ComOff/ChnNonLinearFitY.htm`*
